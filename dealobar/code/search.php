...
include("databaseadapter.php");
$querytogetproducts="SELECT * FROM products WHERE name LIKE '%".$query."%' LIMIT 0,16;";
$result=mysql_query($querytogetproducts);
if($result){
		
	try{
	while ($res=mysql_fetch_array($result)) {
	echo '<div class="col-sm-2 marginman">
		...
		echo "<h1>Ah. No products found</h1>";
		echo $exp;		
		}
...
$query=$_GET['this'];
$query=mysql_escape_string($query);
$querytogetcats="SELECT * FROM category WHERE category LIKE '%".$query."%' LIMIT 0,16;";	
$result=mysql_query($querytogetcats);				
if($result){
									...
									<div class="productinfo text-center">
										...
										<img  src="'.$res['image'].'" alt="$name" />
			 							...
			 							<p>'.$res["name"].'</p>
											...
											<span class="sellingprice pull-right">'.$res["price"].'</span><br/><br/><br/>
											<span class="comapnylabel">'.$res["parentsite"].'</span><br/> 
											<span class="coupon">'.$coupon.'</span><br/> 
										<a href="#" class="btn btn-default add-to-cart"><i class= "fa fa-shopping-cart"></i>View Deal</a>
									<.div>
									</div>
								</div>
	...
	echo '<div class="col-sm-5"><h1>Ah! oh. We dont kow what is wrong with us <br/>Nothing to be found!</h1></div>';
}
?>
